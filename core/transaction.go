...
func (tx *Transaction) setData(data *pb.Transaction) error {
	tx.data = data
	var err error
...
func (txc *TxCommit) setData(data *pb.TxCommit) error {
	txc.data = data
	return nil
}
...
func (txs *TxList) Marshal() ([]byte, error) {
	data := new(pb.TxList)
	data.List = make([]*pb.Transaction, len(*txs))
	for i, tx := range *txs {
		data.List[i] = tx.data
	}
